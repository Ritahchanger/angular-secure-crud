...
class TodoController {
  public getTodos = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!userId) {
        return res
          .status(400)
          .json({ success: false, message: "Not user id provided" });
      }
      ...
      if (!findUserTodos || findUserTodos.length === 0) {
        return res
          .status(404)
          .json({ success: false, message: "User todos not found" });
      }
  ...
  public deleteTodo = async (
    ...
    try {
      if (!todoId)
        return res
          .status(200)
          .json({ success: false, message: "Enter todo id" });
      ...
      if (!deleteTodo) {
        return res
          .status(200)
          .json({ success: false, message: "The todo was not found" });
      }
      return res
        .status(200)
        .json({
  ...
  public confirmTodo = async (
    ...
    try {
      if (!todoId) {
        return res.status(200).json({ success: false, message: "No todo id" });
      }
